...
This directory contains one-off scripts for the Course Development System (my-app) repo. Scripts follow a **naming-prefix convention** so a reader can tell at a glance whether a script reads, writes, or moves data.

## Naming convention

| Prefix | Purpose | Side effects | When to run |
|---|---|---|---|
| `verify-*.ts` | Repeatable verification (positive/negative checks) | **Read-only** (DB or static) | CI / pre-commit / closeout. Safe to re-run. |
| `audit-*.ts` | Inventory / inventory-style audits | **Read-only** | Periodic / closeout. Safe to re-run. |
...
| `test-*.ts` | Unit / integration tests (legacy naming) | **Read by default**; some may write test fixtures | CI. Safe to re-run. |
...
**Rule of thumb**: any script whose name does NOT start with `verify-` / `audit-` / `diagnose-` / `plan-` / `evaluate-` / `validate-` / `dry-run-` / `trial-` (without env) is **likely write-capable**. Read the script before running it.
...
- [docs/project-script-inventory-k35-a.md](project-script-inventory-k35-a.md) — full inventory with `candidate_for_archive` markers and dangerous-script list
