  ...
  const splitCandidatesById = useMemo(() => {
    ...
    if (reviewResult) {
      for (const row of reviewResult.rows) {
        ...
        const ctx = reviewRawMap.get(row.approvalItemId)
    ...
    return map
  }, [reviewResult, reviewRawMap, resolutionOptions, selectedSemesterId])
...
type ResolutionSectionProps = {
  ...
  // L6-E2E: split candidate map from real detection
  splitCandidatesById: Map<string, Array<{
    candidateId: string
    kind: string
    confidence: number
    requiresManualConfirmation: boolean
                    ...
                    <>
                      {splitCandidates.map((candidate, cIdx) => (
                        ...
                        >
                          ...
                          {/* Real assignment table — built from detection results */}
                          ...
                          {/* Per-candidate confirm radio with REAL candidateId */}
