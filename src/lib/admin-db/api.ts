...
export async function fetchAdminTableRecords(table: string): Promise<DbRecord[]> {
...
export async function fetchAdminTableCounts(): Promise<Record<string, number>> {
  ...
  for (const t of TABLES) {
    try {
      const res = await fetch(`/api/admin/${t.key}`)
      const data = await res.json()
...
export async function fetchEntityOptions(): Promise<{
...
export async function fetchTaskOptions(): Promise<EntityOption[]> {
  const res = await fetch('/api/admin/teachingtask')
  const data = await res.json()
