...
- `src/lib/import/importer.ts`
  ...
  - Rewrote `findMergedClassNames` to be **exact-match-first**:
    ...
    2. `.includes()` fallback with `COHORT_WEAK_MATCH_KEPT (weak-match, kept)` warning
    3. subsequence fallback with `COHORT_WEAK_MATCH_KEPT (subseq-match, kept)` warning
...
│   │      - 1 hit  → use it, emit COHORT_WEAK_MATCH_KEPT (weak-match, kept)
│   │      - ≥2 hits → AMBIGUOUS_CLASSGROUP_MATCH, NOT auto-linked
│   │
│   └── 4. subsequence fallback
│          - 1 hit  → use it, emit COHORT_WEAK_MATCH_KEPT (subseq-match, kept)
│          - ≥2 hits → AMBIGUOUS_CLASSGROUP_MATCH, NOT auto-linked
│
...
- Weak matches (`.includes()` and subsequence) that pass the cohort filter
  emit `COHORT_WEAK_MATCH_KEPT` as a warning (not blocking) and the match is
...
| `COHORT_WEAK_MATCH_KEPT`    | weak fallback hit 1 candidate (kept)                                      | kept + warning   |
