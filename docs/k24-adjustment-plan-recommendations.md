...
## 23. K24-A5 Preferred-Day Priority

> 本节由 K24-A5 (`K24-A5-PLAN-RECOMMENDATION-PREFERRED-DAY-PRIORITY`) 阶段追加。

### 23.1 背景

前端人工验证后新需求：在"一键推荐调课方案"优先设置区增加"优先星期"选择。

### 23.2 实现

- `preferredDayOfWeek?: number | null` 输入（1..5 / null）
- 3-bucket 分桶：`preferredDayPlans` > `sameWeekOtherDayPlans` > `fallbackPlans`
- auto 模式（null）行为 100% 保持 K24-A3
- 每个 plan 新增 `isPreferredDay: boolean`
- result 新增 `preferredDayOfWeek` / `preferredDayAvailable`
- searched 新增 `preferredDayPlanCount` / `sameWeekOtherDayPlanCount`

### 23.3 关键不破坏

- K24-A1 UX（preferredPlanWeek / showAdvancedTools / overflow-y-auto）保留
- K24-A2 cross-week self-conflict gate 保留
- K24-A3 preferredWeek-first auto 模式保留（K24-A5 扩展为 3-bucket）
- K24-A4 time-slots [1..5] 保留
- K23-A 66/66 helper 保留
...
修复后需重新做前端人工验证。K24 closeout 暂停 — 验证通过后重新进入 K24-PLAN-RECOMMENDATION-ACCEPTANCE-CLOSEOUT (使用最新 HEAD)。
